...
Show `[optional]` — pressing Enter with no input skips it. If the user
provides a path, it becomes the `input_file` frontmatter field (passed
as the `input_file` env var to the routine).
...
```
Message body [optional, empty line to submit]:
>
```
...
If `input_file` was provided in step 3, show `[optional]` since the
spec file is the primary source of work. If no `input_file`, show
`[recommended]` instead.
...
4. Exclude standard params (`message_file`, `input_file`, etc.)
...
- [ ] Input file prompt shows `[optional]` and accepts empty input
...
- [ ] Body shows `[optional]` when input_file provided, `[recommended]` otherwise
...
- [ ] Unknown routine shows error with available list
- [ ] `decree verify` runs all pre-checks and reports summary
- [ ] `decree verify` exits 0 when all pass, 1 when any fail
