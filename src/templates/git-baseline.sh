...
# On the final attempt, stashes failed changes and restores baseline.
...
if [ "$ATTEMPT" -eq 1 ]; then
    ...
    # Final attempt: save failed state, restore baseline for clean slate
    ...
    if [ -n "$BASELINE_IDX" ]; then
        ...
        echo "Restored baseline for final attempt"
    fi
fi
