...
The `routine:` frontmatter field is **required** — always set it explicitly.
Check `.decree/config.yml` for available routines, or run `decree routine` to
list them. Do not omit `routine:` even when a default is configured.
