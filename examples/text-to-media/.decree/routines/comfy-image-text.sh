#!/usr/bin/env bash
# ComfyUI Image (Text Only)
#
# Generates an image via the ComfyUI FLUX2 workflow using only a text
# prompt (no reference image). The prompt text is read from the message
# body. Image dimensions and output prefix are passed as frontmatter.
#
# Custom parameters:
...
TEMPLATE="$SCRIPT_DIR/../../workflows/image_flux2_text_landscape.json"
